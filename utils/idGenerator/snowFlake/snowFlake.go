...
const (
	workerBits     uint8 = 7  // 每台机器(节点)的id位数 10位最大可以有2^7=128个节点(0-127)
	dataCenterBits uint8 = 3  // 数据中心节点数 8个
	...
	epoch int64 = 1609430400 //这个是我在写epoch这个变量时的时间戳(秒)
)
...
type Worker struct {
	...
	workerIdAssigner worker.WorkerIdAssigner //workId分配实现
}
...
func NewWorker(workerId int64, dataCenterId int64) (*Worker, error) {
	...
	if dataCenterId < 0 || dataCenterId > dataCenterMax {
		return nil, errors.New("datacenterId is invalidate")
	}
	return &Worker{
		timestamp:    0,
...
func NewWorkerAssigner(workerIdAssigner worker.WorkerIdAssigner, dataCenterId int64) (*Worker, error) {
	// 要先检测workerId是否在上面定义的范围内
...
func (w *Worker) GetId() int64 {
	...
	id := int64((now-epoch)<<timeShift | (w.dataCenterId << dataCenterShift) | (w.workerId << workerShift) | (w.number))
	return id
}
